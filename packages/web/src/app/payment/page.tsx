...
function PaymentForm() {
  ...
  async function handlePay(e: React.SyntheticEvent) {
    ...
    if (!token) {
      setError(t('payment.errInvalidLink'));
      return;
    }
    setError('');
    setLoading(true);
          ...
          <div className="lg:col-span-2">
            <div className="rounded-2xl border bg-muted/40 p-6">
              ...
              {isDroplet && plan && (
                <div className="mb-4 rounded-lg border bg-background p-4 text-sm">
                  <div className="grid grid-cols-2 gap-y-2">
                    <span className="text-muted-foreground">{t('payment.specMemory')}</span>
                    <span>{plan.mem}</span>
                    <span className="text-muted-foreground">{t('payment.specVcpus')}</span>
                    <span>{plan.cpu}</span>
                    <span className="text-muted-foreground">{t('payment.specSsd')}</span>
                    <span>{plan.ssd}</span>
                  </div>
                </div>
              )}
              ...
              {!isDroplet && (
                <div className="mb-4 rounded-lg border bg-background p-4 text-sm">
                  <div className="grid grid-cols-2 gap-y-2">
                    <span className="text-muted-foreground">{t('payment.trainingAssistant')}</span>
                    <span>{t('payment.trainingAssistantVal')}</span>
                    <span className="text-muted-foreground">{t('payment.trainingTraining')}</span>
                    <span>{t('payment.trainingTrainingVal')}</span>
                    <span className="text-muted-foreground">{t('payment.trainingAccess')}</span>
                    <span>{t('payment.trainingAccessVal')}</span>
                  </div>
                </div>
              )}
              ...
              <div className="flex items-center justify-between border-t pt-4">
                ...
                <span className="text-2xl font-bold">{isDroplet ? (plan?.mo ?? '—') : '—'}</span>
              </div>
                ...
                <Button type="submit" size="lg" className="w-full mt-2" disabled={isLoading}>
                  {isLoading ? (
                    ...
                    <>
                      ...
                      {t('payment.pay', { amount: isDroplet ? (plan?.mo ?? '') : '' })}
                    </>
                  )}
                </Button>
